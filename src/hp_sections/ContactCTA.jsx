...
export default function ContactCTA() {
  const { localizedPath, ui } = useLocale();
  ...
  return (
    ...
    >
      <HomeStickyKicker>{ui.contactCta.kicker}</HomeStickyKicker>
      ...
      >
        <div className="relative overflow-hidden rounded-card border border-border-strong bg-surface-inverse px-8 py-12 text-fg-on-dark shadow-card sm:px-11 sm:py-14 md:px-14 md:py-16">
          ...
          <div className="relative grid gap-8 md:grid-cols-[minmax(0,1fr)_auto] md:items-center md:gap-10 lg:gap-14">
            <div className="md:pr-4">
              ...
              >
                {ui.contactCta.heading}
              </h2>
              <p className="site-prose-measure mt-5 font-sans text-base font-normal leading-relaxed text-fg-on-dark/88">
                {ui.contactCta.body}
              </p>
            </div>
            <div className="flex flex-col items-stretch gap-3 self-start md:items-end md:self-center">
              <NavLink to={localizedPath('/contact')} className="site-btn-ghost-on-dark">
                <Mail className="h-4 w-4 shrink-0" strokeWidth={2} aria-hidden />
                {ui.contactCta.button}
              </NavLink>
              <p className="site-cta-mono-hint text-center md:text-right">
                {ui.contactCta.routePrefix}
